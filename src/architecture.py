...
def create_pretrained_model(data):
    if cfg.BASE_MODEL == 'densenet121':
        model = cnn_learner(data, models.densenet121, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'densenet169':
        model = cnn_learner(data, models.densenet169, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'densenet201':
        model = cnn_learner(data, models.densenet201, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'nasnetlarge':
        model = applications.NASNetLarge(
            weights="imagenet", include_top=False, input_shape=image_dimensions)
    elif cfg.BASE_MODEL == 'resnet50':
        model = cnn_learner(data, models.resnet50, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'resnet101':
        model = cnn_learner(data, models.resnet101, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'resnet152':
        model = cnn_learner(data, models.resnet152, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'vgg16':
        model = cnn_learner(data, models.vgg16_bn, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'vgg19':
        model = cnn_learner(data, models.vgg19_bn, metrics=cfg.METRIC)
    elif cfg.BASE_MODEL == 'xception':
        model = cnn_learner(data, models.xception, metrics=cfg.METRIC)
    else:
